...
async function carregarQuestoes() {
    const resposta = await fetch("quiz-data.json");

    if (!resposta.ok) {
        throw new Error("Falha ao carregar os dados da prova.");
    }
...
class OnlineQuiz extends HTMLElement {
    ...
    connectedCallback() {
        ...
        this.carregarDadosQuiz();
    }

    async carregarDadosQuiz() {
        try {
            ...
            this.shadowRoot.innerHTML = `
                ...
                <div class="erro">
                    Não foi possível carregar a prova. Verifique se o arquivo
                    <strong>quiz-data.json</strong> está disponível e recarregue a página.
                </div>
            `;
            console.error(erro);
        }
